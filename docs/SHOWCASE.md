...
- 建立 `Relay Labs / 平台研发组`，由陈静负责，工程负责人指定为李伟；
...
- 由陈静向李伟与 Codex 副驾发送一条等待回执的生产放行指令；
- 生成一份带内容摘要的 Office Release Request，等待陈静点击 `放行发布`；
...
- `Q3 客户发布说明`：草案已经提交，同时有一封客户邮件等待陈静放行；
- `生产值班手册`：所有任务已经完成并安全落地。

点击 `成员 ROSTER` 可以看到成员产能、终端和工作日历。Human 使用工作日时间，副驾默认保持 24×7；
这两个维度会同时进入匹配理由和工期计算，而不是把“80% 产能”误当成全天在线。

### 2. 任务流：从 PRD 到具体任务

点击 `任务流 FLOWS`，再点击左侧 LLM Gateway。右侧可以查看任务 DAG、Owner、P50/P80、状态和阻塞原因。
点击任务会切换选中项；底栏可继续接单、推进、规划或在 Human 放行后执行。

### 3. 收件箱：Human 与个人 Agent 协作

点击 `收件箱 INBOX`，再点击顶部当前操作人，把操作人从陈静切到李伟。`TOWER COMMS` 会显示
陈静发来的 Secret 轮换指令，同时覆盖发给 Codex 副驾的消息，因为个人 Agent 始终受其 Human Owner
监督。点击底栏 `收到` 后，指令从待办消失，但确认事件仍留在黑匣子中。

选中一个任务并点击 `发消息`，李伟的回传会自动发给 Demand Requester；切回陈静即可在 Inbox 看到。

### 4. 黑匣子：解释 Flow 为什么可追踪

点击 `黑匣子 TIMELINE`。左侧是当前 Flow 的完整事件链，右侧 Flight Deck 显示已放行的远程航班。
需求、审批、消息、执行、阻塞、升级和人工验收都能按因果顺序重建；退出再打开不会丢失。

### 5. 现场推进状态

回到 `任务流`，选中阻塞的鉴权任务并把操作人切回陈静，点击 `改派`。弹窗会列出满足 `backend/security`
硬能力约束的 Human、Agent 或团队；点击候选条可以改派，输入原因后确认。任务会向新 Owner 重新
...
接着点击左侧 LLM Gateway Flow，把操作人切回陈静并点击 `变更`，输入“增加客户迁移检查
...
对于等待验收的发布文档 Flow，切回陈静、选中已提交任务并点击 `验收`，`WAITING HUMAN` 会随之下降，
...
`demo --showcase`、`dashboard --as 陈静` 和 `GET /api/v1/dashboard` 仍然保留，供测试脚本、Web Console
