...
> 把需求接进来，分给合适的人和 Agent，一路追到验收。

Relay 是一个使用 **Rust** 构建的企业级 **Human-Agent Flow** 平台。管理者只需要提出
...
> Workspace 写入。GitLab 写操作只在 Human Gate 后执行且不包含 merge；Remote Worker 默认在有资源上限、
> 断网、只读根文件系统的 Docker 容器中运行，并继续使用隔离 Git worktree 承载事务性变更。
> 请先在测试仓库和非敏感数据上使用。
...
Relay 不是聊天机器人外壳，也不是只在一个对话里拉起几个 sub-agents。它管理的是一条可能
...
Relay 创建 `FlowRun F-24`，然后执行以下流程。
...
10:02  Coding Flight H-08 took off; pilot=李伟
...
```text
[RELAY_OUT] Flow F-24 completed. All assignments settled.
```

这条从需求到交付、跨管理者、工程师和 Agent 的持久化因果链，就是 Relay 中的 Flow。

## 产品架构

```text
┌──────────────────────────────── Enterprise Surfaces ────────────────────────────────┐
│ Web Console · relay CLI · Feishu · Slack · Teams · Email · API                    │
└───────────────────────────────────────┬─────────────────────────────────────────────┘
...
├── application/   # RelayApp、规划、匹配、排期、Tracker 与管理看板
...
每个员工的 Personal Agent、记忆、凭据、工作区和授权都留在本人边界内。管理者和同事可以发来
`WorkRequest`，但不能直接接管他的 Agent。负责人拿到需求后做拆解和分派：边界清晰的任务可以
交给 Agent 独立完成，需要经验判断的部分由员工和自己的 Personal Agent 配合处理。谁接到
Assignment，谁就对这段工作负责。

负责人不需要包办执行。一份结构化的 `ASSIST` 加一条可验证的 Evidence，通常比把完整上下文
同步给所有人更有效。Flow 可以在任意时刻被 Scheduler 唤醒，但缺少权限、Fuel、Lease 或终止
条件时，任何执行都不会启动。
...
Relay 不允许 LLM 随口给出一个精确日期。工期由多个可解释部分组成：
...
员工与 Personal Agent 的默认分工是：Human 保留方向、判断和授权，Agent 承担检索、
执行与整理。
...
企业需要全流程可观测，但“可观测”不等于所有人默认读取所有秘密。Relay 区分两层记录：
...
Relay Core 不把某个工作领域写死。企业通过能力包让 Human 和 Agent 参与不同类型的 Flow。
...
`coding` Pack 复用隔离 Git worktree，交付物由服务端按变更路径分类为 `code`、`document` 或其他
Artifact，整份 binary patch 的 SHA-256 仍作为完整性证据。Worker PASS 明确禁止 push、merge、修改
remote 和读取凭据，最终接纳补丁由 Human 完成。
...
```bash
relay task authorize TSK-xxxxxxxx \
  --by "工程师 A" --agent "工程师 A 的 Codex" \
  --executor codex --pack office
```

Office Worker 会在销毁隔离 worktree 前把产物暂存为按 SHA-256 寻址的不可变 Artifact；SQLite 和
PostgreSQL 保存同一份内容，另一台 Control Plane 副本也能读取。Agent 可以提交文件上传、邮件发送或
日历创建的 Release Request，但拿不到 OAuth Token，也不能自行发布。只有该 Flow 的 Human Demand
Requester 能批准或驳回，Tower 才会从 Secret 调用 Microsoft Graph 或 Google Workspace API。

邮件等不可幂等副作用在网络结果不确定时进入 `indeterminate`，不会无脑重发；文件覆盖和带稳定 ID 的
日历事件才能由 Tower 恢复。完整配置、最小 Scope 和请求示例见 [Office Human Release Gate](docs/OFFICE.md)。
...
安装器会启动 PostgreSQL 和 Relay，询问真实组织、管理员与团队信息，输出一次性登录 Token；不会
...
```bash
./deploy/install.sh \
  --database-url-file /secure/path/relay-database-url
```
...
源码开发需要 Rust stable。构建后会得到一个 `relay` 二进制：
...
./target/release/relay --data-dir .relay-showcase \
  tui --workspace .
```
...
```bash
./target/release/relay org chart

./target/release/relay demand create \
  "这周完成一个 LLM Gateway" \
  --requester 陈静 \
  --planner local
```
...
```bash
./target/release/relay flow show FLOW-xxxxxxxx
./target/release/relay flow approve FLOW-xxxxxxxx --by 陈静
./target/release/relay inbox --for 李伟

./target/release/relay task accept TSK-xxxxxxxx --by 李伟
./target/release/relay task negotiate TSK-xxxxxxxx --by 李伟 --hours 12
./target/release/relay task start TSK-xxxxxxxx --by 李伟
```
...
```bash
./target/release/relay flow change-propose FLOW-xxxxxxxx \
  "增加客户迁移检查清单" --by 陈静
./target/release/relay flow changes FLOW-xxxxxxxx --as 陈静
./target/release/relay flow change-approve CHG-xxxxxxxx --by 陈静

# 也可以保留原因后驳回
./target/release/relay flow change-reject CHG-xxxxxxxx \
  --by 陈静 --reason "本次发布不需要"
```
...
的窗口、Flow P50/P80 与关键路径。需求发起人也可以先查看满足硬能力约束的候选人，再执行改派：

```bash
./target/release/relay task reassignment-candidates TSK-xxxxxxxx --by 陈静
./target/release/relay task reassign TSK-xxxxxxxx \
  --by 陈静 --to "工程师 B" \
...
```bash
./target/release/relay principal calendar set \
  --for 李伟 --utc-offset +08:00 \
  --days mon,tue,wed,thu,fri --start 09:00 --end 18:00

./target/release/relay principal calendar time-off-add \
  --for 李伟 \
  --from 2026-07-20T09:00:00+08:00 \
  --until 2026-07-22T09:00:00+08:00 \
  --reason "客户现场支持"

./target/release/relay principal calendar show --for 李伟
./target/release/relay principal calendar time-off-cancel \
  --for 李伟 OFF-xxxxxxxx
```

第一版使用显式固定 UTC 偏移，不猜测夏令时和地区节假日；后续 Calendar Connector 会把真实忙碌区间
同步为同一种 append-only 事件。Ratatui 的成员页直接显示每位成员的工作段和有效请假数量。
...
./target/release/relay notification endpoint-add \
  --name operations \
...
./target/release/relay notification deliveries
./target/release/relay notification dispatch --force
```

`relay serve` 默认每 15 秒投递一次，失败后指数退避；Endpoint 停用时未发送记录会转为 `cancelled`，
...
```bash
./target/release/relay message send FLOW-xxxxxxxx \
  "确认生产 Secret 轮换边界并回传结论" \
  --task TSK-xxxxxxxx --by 陈静 \
  --to 李伟 --to "Codex 副驾"

./target/release/relay message inbox --for 李伟
./target/release/relay message ack MSG-xxxxxxxx --by 李伟
./target/release/relay message thread FLOW-xxxxxxxx --as 陈静
```

`command`、`question`、`update` 和 `decision` 都是 Flow Ledger 事件。Requester 可以把新成员拉进 Flow；
其他参与者只能向已经在 Flow 中的人或团队发送消息，避免普通任务成员任意扩大信息可见范围。

任务有未完成的前置依赖时不能开工。交付前必须附上 Evidence，最终 `complete` 只能由已注册的 Human
执行：

```bash
./target/release/relay task evidence TSK-xxxxxxxx \
  --by 李伟 --kind document --uri docs/design.md --summary "方案与验证记录"
./target/release/relay task submit TSK-xxxxxxxx --by 李伟
./target/release/relay task complete TSK-xxxxxxxx --by 陈静
./target/release/relay timeline FLOW-xxxxxxxx
```
...
同事不需要共享 `.relay/flow.db`。本机管理员可以为每个 Principal 签发独立 Token，再启动 HTTP
Control Plane；远程请求会以 Token 对应的真实身份调用同一套 Assignment 和状态门禁：

```bash
./target/release/relay principal token issue \
  --for 李伟 --label "workstation"

./target/release/relay serve --bind 127.0.0.1:7777
```

Token 只显示一次。Ledger 只记录 credential ID、Principal 和签发/撤销事件；SQLite 凭据表只保存
SHA-256 摘要，不保存原始 Token。新 Token 默认 30 天到期，可用 `--ttl-days 1..365` 调整；旧版 v2
数据库会原地迁移到 schema v4，已有 Token 保持兼容。新 Token 包含可路由的 Tenant ID 和 256 位随机
Secret；Tenant ID 不是授权，目标 Ledger 仍会校验完整 Token 摘要、有效期和 Principal 状态。客户端使用
标准 Bearer Header：
...
curl -X POST -H "Authorization: Bearer $RELAY_TOKEN" \
...
```bash
relay principal role list --for 李伟 --by 陈静
relay principal role grant --for 李伟 --role manager --by 陈静
relay principal role revoke ROLE-xxxxxxxx --by 陈静
```

角色授予、撤销和旧 Ledger 的权限迁移都会进入事件流。`manager` 可以创建 Demand 和读取管理看板，
`auditor` 只能读取管理与审计视图，普通 `member` 仍只能操作自己的 Assignment 和 Flow 对话。

一个 Control Plane 可以承载多个隔离 Tenant。根目录的现有 `flow.db` 会自动登记为 `default`，其他
Tenant 各自使用独立事件账本；CLI 通过 ID 或 slug 选择，`relay serve` 会一次加载所有活动 Tenant：

```bash
relay --data-dir /var/lib/relay tenant create \
  --name "Relay APAC" --slug relay-apac
relay --data-dir /var/lib/relay tenant list
relay --data-dir /var/lib/relay --tenant relay-apac org show
relay --data-dir /var/lib/relay serve
```

租户目录由 `control.db` 登记，业务事件仍只写入根 `flow.db` 或 `tenants/<slug>/flow.db`。HTTP 根据 Bearer
Token 的 Tenant ID 选择账本，再在该账本内完成认证；旧格式 Token 只路由到 `default`。签名 Webhook
没有 Bearer Token 时可以使用 `x-relay-tenant: TEN-xxxxxxxx` 选路，但仍必须通过对应 Connector 的签名
验证。Bearer Token 与 Header 指向不同 Tenant 时请求会被拒绝。

多副本部署使用共享 PostgreSQL。`RELAY_DATABASE_URL` 启用 PostgreSQL Tenant Catalog、按 Tenant 分区的
事件流、stream head 行锁和共享 Credential Store；每个请求会先回放目标 Tenant 的最新事件，再进入相同
RBAC 与状态门禁：

```bash
export RELAY_DATABASE_URL='postgresql://relay@db.internal/relay?sslmode=require'
relay --data-dir /var/lib/relay org init --name "Relay Holdings"
relay --data-dir /var/lib/relay serve --bind 127.0.0.1:7777
```

多个 `relay serve` 副本可以连接同一数据库。提交事务先锁定该 Tenant 的 stream head；过期副本会收到
sequence conflict、回放后再由调用方重试，不会产生分叉事件。`--data-dir` 在此模式下只保存每个副本的
本地运行产物，不再是业务状态源。数据库 URL 必须来自环境或 Secret Manager，不能写入 Flow、CLI 参数
或仓库。

从现有 SQLite Fleet 迁移时，保持 `RELAY_DATABASE_URL` 未设置，并把目标放在独立环境变量。命令会迁移
全部 Tenant、事件 sequence/ID/时间、Token 摘要与撤销状态，以及不可变 Office Artifact；重复运行只做
一致性复核：

```bash
export RELAY_TARGET_DATABASE_URL='postgresql://relay@db.internal/relay?sslmode=require'
relay --data-dir /var/lib/relay ops migrate-postgres
```
...
| Endpoint | 作用 |
| --- | --- |
| `GET /console` | 内嵌管理员 Web Console；支持 OIDC Session 或 Principal Bearer Token |
| `GET /health` | 无认证健康检查 |
| `GET /health/live` | 进程存活检查，不访问组织数据 |
| `GET /health/ready` | SQLite quick-check、Schema 和组织初始化就绪检查 |
| `GET /auth/oidc/login` | 发起 Authorization Code + PKCE 企业登录 |
| `GET /auth/oidc/callback` | 校验 state、nonce、ID Token 后签发 HttpOnly Session |
| `POST /auth/logout` | 撤销当前 OIDC Session 并清除 Cookie |
| `/scim/v2/Users[/:id]` | SCIM 2.0 Human 查询、预置、更新、停用和删除 |
| `/scim/v2/Groups[/:id]` | SCIM 2.0 Team 查询、成员同步、停用和删除 |
| `/scim/v2/{ServiceProviderConfig,ResourceTypes,Schemas}` | SCIM 服务能力发现 |
| `GET /metrics` | 需要 Dashboard 权限的 Prometheus 指标 |
...
| `GET/POST /api/v1/flows/:id/messages` | 读取有权查看的对话或向团队、Human、Agent 发送消息 |
...
```bash
./target/release/relay principal token list --for 李伟
./target/release/relay principal token revoke CRED-xxxxxxxx
```

`relay serve` 默认只监听 `127.0.0.1`。非 loopback 明文地址默认拒绝启动；确认前方已有可信 TLS
终止后，才可显式传 `--allow-insecure-public-http`。当前版本已有事件化组织 RBAC、单进程 SQLite 多
Tenant 隔离、PostgreSQL 多副本共享数据面、OIDC SSO 和 SCIM 生命周期，但还没有应用内 TLS、集中策略
引擎或分布式限流；进程内仅提供每个 Token/匿名键每分钟 300 次的基础保护。跨机器部署必须放在带
更严格限流的 TLS 反向代理或可信内网后面。签发 Token 的本地 CLI `admin` 操作者仍被视为引导期
Control Plane 管理员。

单节点 Ledger 使用 SQLite WAL、`synchronous=FULL`、5 秒 busy timeout 和严格 schema 版本检查；Unix
数据目录与数据库分别收紧为 `0700` / `0600`。日常检查与在线一致性快照：

```bash
relay --data-dir /var/lib/relay ops doctor
relay --data-dir /var/lib/relay ops backup \
  --output /var/backups/relay/flow-$(date -u +%Y%m%dT%H%M%SZ).sqlite
```

备份不会覆盖已有文件，生成后立即执行 `quick_check`。完整的 TLS、健康检查、指标、Secret、备份和恢复
步骤见 [生产部署手册](docs/DEPLOYMENT.md)。
...
```bash
./target/release/relay principal add \
...
在工程师自己的工作站上保留 Token、仓库，并先构建固定版本的 Agent Runtime。容器默认断网；下面的
Codex 云端规划显式打开网络并只注入模型凭据：

```bash
docker build -f docker/agent-runtime/Dockerfile \
...
云端 Claude Code / Codex 需要显式打开 `bridge` 网络并通过 `--sandbox-env` 白名单注入短效凭据；默认不会
挂载宿主机 Home、CLI 登录状态或 Docker socket。生产节点还要在主机网络层限制出站目的地。本地兼容
调试可显式使用 `--sandbox process`，Dashboard 会把它标成 `process/host`，该模式不能作为生产安全边界。
镜像构建、资源参数、凭据边界和验收清单见 [Remote Worker 容器沙箱](docs/WORKER_SANDBOX.md)。
...
```bash
git apply --check .relay/worker-runs/TSK-*/WRUN-*/changes.patch
git apply         .relay/worker-runs/TSK-*/WRUN-*/changes.patch
```
...
relay task recover LEASE-xxxxxxxx \
...
GitLab 是第一条外部工作事实源。Relay 使用只读 REST API 检查项目，并把某个 Merge Request 与
...
./target/release/relay gitlab check --project platform/llm-gateway
./target/release/relay gitlab sync \
  --task TSK-xxxxxxxx \
...
只读连接器读取 Project、单个 MR 及其 Pipeline，也支持 Webhook 自动刷新。另有独立的 Human-gated
Writer 创建/评论 Issue 和创建/评论 MR；它使用单独的 Tenant Token，绝不提供 merge 动作。配置、审批、
不确定结果恢复和 API 示例见 [GitLab 写协作](docs/GITLAB_WRITES.md)。自托管实例也可以给只读 CLI
...
```bash
./target/release/relay track scan --stale-hours 24 --escalate-after-hours 4
./target/release/relay track list
./target/release/relay track list --flow FLOW-xxxxxxxx --all
./target/release/relay track inbox --for 陈静
./target/release/relay track ack ESC-xxxxxxxx --by 陈静
```
...
```bash
./target/release/relay executor check claude-code
./target/release/relay executor check codex
```

也可以不使用演示数据，显式建立组织关系：

```bash
./target/release/relay org init --name "Acme"
./target/release/relay team add --name Platform \
  --capabilities "rust,backend,llm-platform,security"
./target/release/relay principal add --name "工程师 A" --kind human \
  --team Platform --capabilities "rust,backend,llm-platform,security"
./target/release/relay principal add --name "A 的 Codex" --kind agent \
  --team Platform --owner "工程师 A" \
  --capabilities "rust,backend,llm-platform,security" \
...
```bash
./target/release/relay task run TSK-xxxxxxxx --by "工程师 A" --mode plan
```

只有已分配的 Human 显式传入 `--mode execute`，才允许终端修改注册的工作区：

```bash
./target/release/relay task run TSK-xxxxxxxx --by "工程师 A" --mode execute
```
...
```bash
./target/release/relay demand create "准备 Q3 发布计划" \
...
自定义命令和模型配置；认证由本机已经登录的 CLI 负责，Relay 当前不保存 Anthropic 或 OpenAI
...
- 飞书交互卡片、Slack Block Kit、Teams Adaptive Card 原生通知 Connector 与审计测试消息；
...
- Human-gated GitLab Issue/MR 创建与评论、Tenant Token、事务性 Artifact 和未知结果保护；
- 单进程 Tenant Catalog、独立 Ledger、Tenant Token 路由与 CLI Tenant 选择；
- PostgreSQL 共享 Tenant Catalog、stream-head 并发控制、请求级回放与 SQLite Fleet 幂等迁移；
- OIDC Authorization Code + S256 PKCE、跨副本签名 state、HttpOnly Session 与 SCIM User/Group 生命周期；
- 不可变 Office Artifact、Human Release Gate、Microsoft Graph / Google Workspace 原生写入与不确定结果保护；
...
目前没有内置 OAuth Token 获取/刷新门户、基于 Attention 的自动改派、地区节假日同步、自动 commit/push
或自动 merge。Docker Worker 不是虚拟机，也不内置域名级出站策略；生产环境仍需专用 Worker VM、
rootless Runtime 和网络策略。GitLab Writer 和 Office Bridge 只开放经过 Human Gate 的固定动作，
Access Token 必须由企业 Secret Manager 或 Credential Broker 注入。

## 为什么选择 Rust

Relay 使用 Rust 实现 Control Plane、Execution Runtime 和 CLI：
...
v0 使用 Tokio、Clap、Ratatui、Crossterm、Serde、Schemars、SQLite 和可选 PostgreSQL。后续是否引入
Actor Runtime、NATS 或其他基础设施，将由实际的远程执行与跨地域需求决定。
...
Relay 不是 DeerFlow 的 fork。DeerFlow 可以成为某种 Agent Execution Backend，而 Relay
...
- [ ] 贡献规范与安全披露流程
- [x] Coding Capability Pack
- [x] Office 草稿 Capability Pack
- [x] 不可变 Office Artifact、Human Release Gate 与 Microsoft 365 / Google Workspace Bridge
- [x] 内嵌 Web Console、管理员看板与 Flight 恢复操作
- [x] 单节点生产运维：schema v4、Token TTL、备份恢复、健康检查、指标、限流、CI 与非 root 镜像
...
本项目为内部项目，见 [LICENSE](./LICENSE)。代码、文档和运行数据仅限公司内部使用，不对外分发。
