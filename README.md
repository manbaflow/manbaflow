...
> FlightManifest、Fuel、资源租约和监督树恢复。它还没有生产级容器沙箱、Office 云端写入和 GitLab
> 写操作。请先在测试仓库和非敏感数据上使用。
...
第一版 `office` Pack 已实现“生成可审查草稿”的安全内核：Manifest 默认只允许 `md/txt/html/pdf/docx`、
...
Office Worker 目前只在隔离工作区产出文件，不会直接发邮件、邀请参会人、发布文档或上传云盘。模型无法
把“生成邮件草稿”悄悄扩大为“替 Human 发送邮件”。Microsoft 365、Google Workspace 和企业 MCP 的
外部写操作将通过独立 Bridge、最小 OAuth Scope 和新的 Human Release Gate 接入；当前版本没有把这类
副作用伪装成已完成。
...
数据库会原地迁移到 schema v3，已有 Token 保持兼容。新 Token 包含可路由的 Tenant ID 和 256 位随机
...
全部 Tenant、事件 sequence/ID/时间、Token 摘要和撤销状态；重复运行只做一致性复核：
...
- SQLite v3 迁移、WAL/FULL 同步、文件权限、Doctor、在线快照、readiness 与受认证 metrics；
...
目前没有 GitLab 写操作，也没有 Microsoft 365、Google Workspace 的原生 OAuth 写入 Bridge、基于 Attention
的自动改派、地区节假日同步、生产级容器沙箱、自动 commit/push/MR、自动云端发布。这一版先验证组织
Flow 是否真的比一次 Agent 对话更适合承载跨人的长期工作。
...
- [x] 单节点生产运维：schema v3、Token TTL、备份恢复、健康检查、指标、限流、CI 与非 root 镜像
