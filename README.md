...
终止后，才可显式传 `--allow-insecure-public-http`。当前版本已有事件化组织 RBAC 和单进程多 Tenant
账本隔离，但还不是可横向扩展的共享数据面，也没有 TLS、OIDC SSO、SCIM 或分布式限流；进程内仅提供
...
的自动改派、地区节假日同步、PostgreSQL 多副本数据面、OIDC/SCIM、生产级容器沙箱、自动
...
- [ ] 更多企业连接器
- [ ] PostgreSQL 多副本数据面、SSO/SCIM、策略引擎和生产级隔离
