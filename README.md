...
数据库会原地迁移到 schema v3，已有 Token 保持兼容。客户端使用标准 Bearer Header：
...
终止后，才可显式传 `--allow-insecure-public-http`。当前版本已有事件化组织 RBAC，但一个数据目录仍只承载一个
活动 Tenant，也还没有 TLS、OIDC SSO、SCIM 或分布式限流；进程内仅提供每个 Token/匿名键每分钟
300 次的基础保护。跨机器部署必须放在带更严格限流的 TLS 反向代理或可信
...
自动云端发布。这一版先验证组织 Flow 是否真的比一次 Agent 对话
更适合承载跨人的长期工作。
