...
> Workspace 写入。GitLab 写操作只在 Human Gate 后执行且不包含 merge；它还没有生产级容器沙箱。
> 请先在测试仓库和非敏感数据上使用。
...
目前没有内置 OAuth Token 获取/刷新门户、基于 Attention 的自动改派、地区节假日同步、生产级容器沙箱、
自动 commit/push 或自动 merge。GitLab Writer 和 Office Bridge 只开放经过 Human Gate 的固定动作，
