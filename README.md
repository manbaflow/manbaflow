...
> 远程 Worker、沙箱或 Web Console。HTTP Control Plane 的 Bearer Token 仍是本地单组织方案，不等同于
> 企业 SSO/RBAC。请先在测试仓库和非敏感数据上使用。
...
Claude Code / Codex 副驾；它只写入 `.mambaflow/`，不会启动模型：
...
| `t` | 立即巡航扫描 Todo Attention；TUI 后台每 30 秒自动扫描 |
| `g` | 在 Inbox 确认首个等待响应的 Tower Call |
...
`mamba serve` 默认只监听 `127.0.0.1`。当前版本没有 TLS、SSO、限流或租户 RBAC；跨机器部署必须放在
TLS 反向代理或可信内网后面。签发 Token 的本地 CLI 操作者仍被视为 Control Plane 管理员。
...
在工程师自己的工作站上保留 Token、仓库和已经登录的 Claude Code/Codex。执行一次规划航班：
...
进程在起飞后中断时，可以用 `--task TSK-xxxxxxxx` 显式恢复。

原始 stdout/stderr 黑匣子留在 Worker 的 `.mambaflow/worker-runs/`，塔台当前只能看到摘要和
`worker://` 引用。坠机会回传 `worker_blackbox` Evidence 并阻塞 Task。第一版不开放远程写模式：在
Flight Lease、Human 授权、日志安全上传和 worktree/container 隔离完成前，Remote Worker 不能自动
调用 `workspace-write`，因此它现在是可审计的远程规划员，还不是无人值守 Coding Agent。
...
作用于注册的工作区，使用者必须先检查仓库状态和终端自身的授权配置。
...
- 无需共享数据库或仓库的只读 Remote Worker，以及远程 Agent 身份；
...
目前没有 GitLab 写操作，也没有 Microsoft 365、Google Workspace Connector、基于 Attention 的自动
改派和外部渠道通知、工作日历、租户级 RBAC、动态恢复树、远程写入 Flight Lease、生产级沙箱、
Coding/Office Capability Pack 或 Web Console。这一版先验证组织 Flow 是否真的比一次 Agent 对话
更适合承载跨人的长期工作。
...
- [ ] 贡献规范与安全披露流程
- [ ] Remote Flight Lease、Human 写授权与 Worker 黑匣子上传
