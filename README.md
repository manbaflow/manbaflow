...
Tenant 隔离和 PostgreSQL 多副本共享数据面，但还没有 TLS、OIDC SSO、SCIM 或分布式限流；进程内仅提供
每个 Token/匿名键每分钟 300 次的基础保护。跨机器部署必须放在带更严格限流的 TLS 反向代理或可信
...
的自动改派、地区节假日同步、OIDC/SCIM、生产级容器沙箱、自动
commit/push/MR、自动云端发布。这一版先验证组织 Flow 是否真的比一次 Agent 对话
更适合承载跨人的长期工作。
