...
> Workspace 写入。它还没有生产级容器沙箱和 GitLab 写操作。请先在测试仓库和非敏感数据上使用。
...
目前没有 GitLab 写操作、内置 OAuth Token 获取/刷新门户、基于 Attention 的自动改派、地区节假日同步、
生产级容器沙箱、自动 commit/push/MR 或任意云端发布。Office Bridge 只开放经过 Human Gate 的固定
Graph/Workspace 动作，Access Token 必须由企业 Credential Broker 注入。
