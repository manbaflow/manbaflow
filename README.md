...
> 执行终端适配器。它还没有生产级容器沙箱、企业身份系统或 Web Console；HTTP Control Plane 的 Bearer
> Token 仍是本地单组织方案，不等同于企业 SSO/RBAC。请先在测试仓库和非敏感数据上使用。
...
| `m` | 围绕当前任务向 Owner、Copilot 或 Requester 传球 |
| `i` | 协商当前任务工时并动态重排整条 Task DAG |
| `v` | Demand Requester 为当前任务选择新 Owner 并重新排期 |
| `f` | 为进行中的 Flow 提出追加范围，生成新任务和 P80 影响预览 |
| `a` / `o` | 批准待处理变更 / 输入原因驳回变更；批准前正式 Flow 保持不变 |
| `t` | 立即巡航扫描 Todo Attention；TUI 后台每 30 秒自动扫描 |
| `g` | 在 Inbox 确认首个 Flow 指令或 Tower Call |
...
改派和外部渠道通知、工作日历、租户级 RBAC、动态恢复树、生产级容器沙箱、自动 commit/push/MR、
