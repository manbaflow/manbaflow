...
事件。自动重新估时、外部通知、升级和重新匹配仍属于后续策略层。
...
或验收视为需要关注；Blocked 和超过 P80 会直接成为 Critical：

```bash
./target/release/mamba track scan --stale-hours 24
...
目前没有基于 Attention 的自动改派、外部通知与升级策略，也没有工作日历、租户级 RBAC、动态恢复树、远程 Worker、生产级沙箱、
Coding/Office Capability Pack 或 Web Console。这一版先验证组织 Flow 是否真的比一次 Agent 对话更
适合承载跨人的长期工作。
...
- [x] 基础 Todo Tracker 与可回放 Attention
