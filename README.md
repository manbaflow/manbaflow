...
Tenant 各自使用独立事件账本；CLI 通过 ID 或 slug 选择，`mamba serve` 会一次加载所有活动 Tenant：
...
mamba --data-dir /var/lib/manbaflow --tenant mamba-apac org show
...
没有 Bearer Token 时可以使用 `x-mamba-tenant: TEN-xxxxxxxx` 选路，但仍必须通过对应 Connector 的签名
验证。Bearer Token 与 Header 指向不同 Tenant 时请求会被拒绝。

多副本部署使用共享 PostgreSQL。`MAMBA_DATABASE_URL` 启用 PostgreSQL Tenant Catalog、按 Tenant 分区的
...
mamba --data-dir /var/lib/manbaflow org init --name "Mamba Holdings"
mamba --data-dir /var/lib/manbaflow serve --bind 127.0.0.1:7777
```

多个 `mamba serve` 副本可以连接同一数据库。提交事务先锁定该 Tenant 的 stream head；过期副本会收到
...
从现有 SQLite Fleet 迁移时，保持 `MAMBA_DATABASE_URL` 未设置，并把目标放在独立环境变量。命令会迁移
...
`mamba serve` 默认只监听 `127.0.0.1`。非 loopback 明文地址默认拒绝启动；确认前方已有可信 TLS
...
```bash
mamba --data-dir /var/lib/manbaflow ops doctor
mamba --data-dir /var/lib/manbaflow ops backup \
  --output /var/backups/manbaflow/flow-$(date -u +%Y%m%dT%H%M%SZ).sqlite
```
